const CACHE='daylist-offline-v2';
...
self.addEventListener('activate',event=>{event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key!==CACHE&&key.startsWith('daylist-offline-')).map(key=>caches.delete(key)))).then(()=>self.clients.claim()));});
